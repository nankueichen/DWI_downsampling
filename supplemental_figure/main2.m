...
lineLengthShort = zeros(size(KShort,1),3);
lineLengthLongMatchWithShort = zeros(size(KLongMatchWithShort,1),3);
...
for cnt = 1:size(KShort,1)
    ...
    lineLengthShort(cnt,:)=pdist(tmpABC);
end
for cnt = 1:size(KLongMatchWithShort,1)
    ...
    tmpABC = [tmpA; tmpB; tmpC];
    lineLengthLongMatchWithShort(cnt,:)=pdist(tmpABC);
end
...
%% This section is used to produce the Supplemental figure
% Comparing the spatial uniformity measured by 1) standard deviation of the
% Delaunay triangle areas and 2) sum of repulsive force, using 1000 randomly 
% chosen subsets of bvec

stdSaveMeA = zeros(1000,1);
stdSaveMeD = zeros(1000,1);
for cnt2 = 1:1000
    randomList = randperm(length(L_Long));
    randomList = randomList(1:length(L_Short));
    vecLongMatchWithShortRandom = vecLong(:,randomList);
    ...
    tmp10 = std(areaLongMatchWithShortRandom);
    stdSaveMeA(cnt2,1) = tmp10;
    tmp002 = sort(lineLengthLongMatchWithShort(:));
    tmp002 = tmp002(1:4:end);
    tmp002 = tmp002.^2;
    tmp002 = 1./tmp002;
    stdSaveMeD(cnt2,1) = sum(tmp002);

end
figure;
plot(stdSaveMeA, stdSaveMeD, 'bo', 'LineWidth', 1.);
xlabel('standard deviation of Delaunay triangle areas (arbitrary unit)'); 
ylabel('sum of repulsive force (arbitrary unit)')
